...
require(dirname(__FILE__) . '/includes/init110.php');
...
if ($_REQUEST['act'] == 'list')
{
	...
	{
		$stations = $db->getAll("SELECT station_id,station_name FROM ship_station where station_id <10 ");
		$smarty->assign('stations',   $stations);
	}
...
function order_list()
{
   if (!empty($_GET['is_ajax']) && $_GET['is_ajax'] == 1)
   {
	...
	$sql = "select *,a.status as status,p.print_sn,o.address as address ".
    ...
    $arr = array('orders' => $res, 'filter' => $filter, 'page_count' => $page_count,'record_count' => 30);
