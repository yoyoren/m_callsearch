...
if ($_REQUEST['act'] == '')
{
	...
	$smarty->display('order_bstat2.html');
}
elseif ($_REQUEST['act'] == 'top')
{

    $smarty->display('bstat_top2.html');
}
...
elseif ($_REQUEST['act'] == 'list')
{
	...
	$smarty->display('border_stat_list.html');
}
...
function order_list()
{
	...
	$filter['pp']    = empty($_REQUEST['pp'])    ? 0  : intval($_REQUEST['pp']);	
	//$filter['pound']    = isset($_REQUEST['pound'])    ? 1  : 0;	
	$filter['goods_id'] = empty($_REQUEST['goods_id']) ? 0  : intval($_REQUEST['goods_id']);
	$filter['goods_attr'] = empty($_REQUEST['goods_attr']) ? 0  : $_REQUEST['goods_attr'];	
	//print_r($_REQUEST);
	...
	if($filter['pp'])
	{
	   $where .= " and gg.goods_price >600 ";
	   $join  .= " left join ecs_order_goods as gg on b.order_id=gg.order_id ";
	   $str   .= ",gg.goods_attr ";
	}
	if($filter['goods_id'])
	{
	   $where .= " and g.goods_id = ".$filter['goods_id'];
	   $join  .= " left join ecs_order_goods as g on b.order_id=g.order_id ";
	   $str   .= ",g.goods_attr ";
	}	
	
	if($filter['goods_attr'])
	{
	   $where .= " and ggg.goods_attr = '".$filter['goods_attr']."磅'";
	   $join  .= " left join ecs_order_goods as ggg on b.order_id=ggg.order_id ";
	   $str   .= ",ggg.goods_attr ";
	}	

    $sql = "SELECT COUNT(*) FROM order_genid AS b left join ecs_order_info as a on a.order_id=b.order_id ".
	       "left join print_log as c on a.order_id=c.order_id ".$join. $where;
	...
	$sql = "SELECT b.*,a.country,a.province,a.city,a.order_sn,a.add_time,a.best_time,a.order_status,a.pay_status,".
	       "a.order_amount,a.address,a.orderman,a.ordertel,c.ptime,c.stime".$str." FROM order_genid as b ".
	       "left join ecs_order_info as a on a.order_id=b.order_id ".$join.
	...
	{
	   $sql = "SELECT b.name, b.office_phone FROM order_delivery AS a, hr_employees AS b WHERE a.employee_id = b.id AND order_id =".$val['order_id'];
