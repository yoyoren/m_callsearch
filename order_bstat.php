...
if ($_REQUEST['act'] == '')
{
	...
	$smarty->display('order_bstat.html');
}
elseif ($_REQUEST['act'] == 'top')
{

    $smarty->display('bstat_top.html');
}
...
elseif ($_REQUEST['act'] == 'list')
{
	...
	$smarty->display('border_stat_list2.html');
}
...
function order_list()
{
	...
	$sql = "SELECT b.*,a.country,a.province,a.city,a.order_sn,a.add_time,a.best_time,a.order_status,a.pay_status,".
	       "a.order_amount,a.address,a.orderman,a.ordertel,c.ptime,c.stime FROM order_genid as b ".
	       "left join ecs_order_info as a on a.order_id=b.order_id ".
		   "left join print_log_bt as c on a.order_id=c.order_id ".$where.
	...
	{
	   $sql = "SELECT t.station_name, b.name, b.office_phone
				FROM order_delivery AS a, hr_employees AS b, order_dispatch AS s, ship_route AS r, ship_station AS t
				WHERE a.employee_id = b.id
				AND a.order_id = s.order_id
				AND s.route_id = r.route_id
				AND r.station_id = t.station_id
				AND a.order_id  =".$val['order_id'];
	   ...
	   $rs[$key]['sender'] = $res['station_name'].$res['name'];
	   $rs[$key]['sendertel'] = $res['office_phone'];
