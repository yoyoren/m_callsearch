...
<form action="order_stat.php?act=list&agt=<?php echo $this->_var['agt']; ?>" method="post" target="list">
<div class="main">
<?php if ($this->_var['agt'] == '2'): ?>
城市：<select name="city"><option value="">沪杭</option>
			<option value="442">上海</option>
			<option value="440">杭州</option>
			<option value="444">苏州</option>
     </select>
<?php else: ?>
城市：<select name="city">
...
订单号：<input type="text" name="order_sn" size="14"></td>
...
订货电话：<input type="text" name="ordertel" size="14">
...
下单时间：<input type="checkbox" onpropertychange="for(i=0;i<T2.length;i++){with(T2[i]){disabled=!this.checked,style.background=!this.checked?'#ececec':'#ffffff'}}" checked>
		...
		<input id="T2" size="10" name="edated" type="text" readonly="true" onFocus="javascript:WdatePicker()" /><br />
...
<br />
订货人称：<input type="text" name="orderman" size="14">&nbsp;订货电话：<input type="text" name="ordertel" size="14">&nbsp;结款地址：<input type="text" name="maddress" size="14"><br />
收货人称：<input type="text" name="consigne" size="14">&nbsp;收货电话：<input type="text" name="contel" size="14">&nbsp;送货地址：<input type="text" name="address" size="14"><br />
