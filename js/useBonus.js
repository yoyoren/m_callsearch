...
$.fn.ready(function() {
    ...
    $("#bonus_sn").autocomplete(url, {
        ...
        formatItem: function(row, i, max) {
            if (row.cardused == "1") {
                return row.CheckCode + "(<span style=\"color:red\">已使用</span>)";
            }else if(row.cardused == "2"){
				return row.CardNumber + "(<span style=\"color:red\">已过期</span>)";
			}else {
                return row.CardNumber + "(<span style=\"color:green\">未使用</span>)";
    ...
    $("#bonus_sn").result(function(event, data, formatted) {
        ...
        $("#bonus_cardnum").val(data.CardNumber);
    });
});
...
function UseGiftCard(card) {
	...
	$("#remark").val($("#remark").val()+"卡号:"+card.CardNumber+";");
    $("#bonus_sn").val("");
    $("#bonus_cardnum").val("");
	var trname = "trbonus" + trcount;
	var bonus=card.CardNumber+":"+card.value;
	var tr="<tr id="+trname+"><td>卡号:"+card.CardNumber+",金额:"+card.value+"<input type='image' src='images/icon_drop.gif' onclick=\"delbonus('#"+trname+"')\"><input type='hidden' value='"+bonus+"' class='hidebonus' name='bonus[]'></td></tr>";
...
function delbonus(rows){
		  ...
		  $("#remark").val($("#remark").val().replace("卡号:"+cardnum+";", ""));
		  $("#bonus").next().val(orderFee.usedBonus);
		  if(orderFee.usedBonus==0) 
		  {
			  $("#bonus").next().val('');
